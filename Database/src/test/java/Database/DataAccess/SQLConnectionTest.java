...
public class SQLConnectionTest {
    ...
    void transfer_is_saved_in_the_database() throws SQLException {
        UpdatedBalancesForTransferDTO transferRequestDTO = new UpdatedBalancesForTransferDTO(1000,2000,"-", "bbbbaaaaccccdddd", "aaaabbbbccccdddd", 20);
    ...
    * @Test
    ...
    @Test
    void checkInterestRate_reaches_the_database() throws SQLException {
        CheckAccountDTO checkAccountDTO = new CheckAccountDTO("-");
        Mockito.when(resultSet.next()).thenReturn(true);
    ...
    void deposit_updates_the_database() throws SQLException {
        DepositRequestDTO depositRequestDTO = new DepositRequestDTO("aaaabbbbccccdddd",50,550);
        sqlConnection.deposit(depositRequestDTO);
        Mockito.verify(connection).prepareStatement("UPDATE account SET balance = ? WHERE account_id = ?");
        ...
        assertEquals("aaaabbbbccccdddd",stringCaptor.getAllValues().get(0));
        assertEquals(550,doubleCaptor.getAllValues().get(0));
        assertEquals(50,doubleCaptor.getAllValues().get(1));
        assertEquals("deposit",stringCaptor.getAllValues().get(1));
        assertEquals("aaaabbbbccccdddd",stringCaptor.getAllValues().get(2));
        assertEquals("aaaabbbbccccdddd",stringCaptor.getAllValues().get(3));
    ...
    void test_CreditInterest_queries_the_database() throws SQLException {
        CreditInterestDTO creditInterestDTO = new CreditInterestDTO("aaaabbbbccccdddd",500,5500);
        Mockito.when(resultSet.next()).thenReturn(true);
        Mockito.when(statement.executeUpdate()).thenReturn(1);
        sqlConnection.creditInterest(creditInterestDTO);
        Mockito.verify(connection).setAutoCommit(false);
        Mockito.verify(connection).prepareStatement("UPDATE account SET balance = ? WHERE account_id = ?");
        Mockito.verify(connection).prepareStatement("INSERT INTO transactions(dateTime, amount, senderAccount_id, recipientAccount_id, transaction_type) VALUES (?, ?, ?, ?, 'Interest')");

        Mockito.verify(statement).setDouble(1,creditInterestDTO.getNewBalance());
        Mockito.verify(statement).setString(2,creditInterestDTO.getAccount_id());
        Mockito.verify(statement,Mockito.times(2)).executeUpdate();

        Mockito.verify(statement).setTimestamp(Mockito.eq(1),Mockito.any(Timestamp.class));
        Mockito.verify(statement).setDouble(2,creditInterestDTO.getAmount());
        Mockito.verify(statement).setString(3,creditInterestDTO.getAccount_id());
        Mockito.verify(statement).setString(4,creditInterestDTO.getAccount_id());
        Mockito.verify(connection).commit();
    }
    ...
    void logLoan_updates_the_db() throws SQLException {
        ...
        Mockito.verify(connection).prepareStatement("INSERT INTO transactions(dateTime, amount, senderAccount_id, recipientAccount_id, transaction_type) "
                +
                "VALUES (?, ?, ?, ?, 'Loan')");
        Timestamp now = Timestamp.valueOf(LocalDateTime.now());
        Mockito.verify(statement).setTimestamp(Mockito.eq(1),Mockito.any(Timestamp.class));
        Mockito.verify(statement).setDouble(2,dto.getLoanAmount());
        Mockito.verify(statement).setString(3,dto.getAccountId());
    ...
    @Test
    public void updateEmail_updates_and_calls_db() throws SQLException {
        UserNewEmailDTO dto = new UserNewEmailDTO(1,"-");
        sqlConnection.updateEmail(dto);
        Mockito.verify(connection).prepareStatement("UPDATE \"user\" SET email = ? WHERE user_id = ?");
        Mockito.verify(statement).setString(1, dto.getEmail());
        Mockito.verify(statement).setInt(2, dto.getUserID());
        Mockito.verify(statement).executeUpdate();
    }
    @Test
    public void updatePassword_updates_and_calls_db() throws SQLException {
        UserNewPasswordDTO dto = new UserNewPasswordDTO(1,"-");
        sqlConnection.updatePassword(dto);
        Mockito.verify(connection).prepareStatement("UPDATE \"user\" SET password = ? WHERE user_id = ?");
        Mockito.verify(statement).setString(1, dto.getPassword());
        Mockito.verify(statement).setInt(2, dto.getUserID());
        Mockito.verify(statement).executeUpdate();
    }
    @Test
    public void updatePlan_updates_and_calls_db() throws SQLException {
        UserNewPlanDTO dto = new UserNewPlanDTO(1,"-");
        sqlConnection.updatePlan(dto);
        Mockito.verify(connection).prepareStatement("UPDATE \"user\" SET plan = ? WHERE user_id = ?");
        Mockito.verify(statement).setString(1, dto.getPlan());
        Mockito.verify(statement).setInt(2, dto.getUserID());
        Mockito.verify(statement).executeUpdate();
    }
